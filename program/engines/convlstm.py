...
class ConvLstmTrainer(Trainer, Evaluator):
        ...
        with torch.no_grad():
            ...
            for i, sample in enumerate(self.eval_loader): 
                ...
                if i == 1 and epoch % 1 == 0: 
                    self.plot(epoch, sample["Input"], output_chunks, target_chunks)
                        
            chunk_losses = {key: [val / len(self.eval_loader.dataset) for val in values] for key, values in self.running_losses.items()}
    ...
    def plot(self, idx, input, output, target):
        seq_len = self.config['seq_length']
        _, ax = plt.subplots(self.rollout_times * 2 + 1, seq_len + 1,
                             figsize=(seq_len * 2 + 2, self.rollout_times * 4 + 2))
        row_titles = ["Input", "Output 1", "Target 1", "Output 2", "Target 2"]
        ...
        for j in range(seq_len):
            ...
            ax[0][j + 1].set_title("Timestep {timestep}".format(timestep=j + 1),
                                   fontsize=10)
        for k in range(self.rollout_times):
            for j in range(seq_len):
                # visualise output
                ax[2 * k + 1][j + 1].imshow(
                    output[k][0][j][0].cpu().detach().numpy())
                ax[2 * k + 1][j + 1].set_xticks([])
                ax[2 * k + 1][j + 1].set_yticks([])
                ax[2 * k + 1][j + 1].set_title("Timestep {timestep}".format(
                    timestep=j + (k + 1) * seq_len + 1),
                                               fontsize=10)
                # visualise target
                ax[2 * k + 2][j + 1].imshow(
                    target[k][0][j][0].cpu().detach().numpy())
        ...
        with torch.no_grad():
            for i, sample in enumerate(self.eval_loader):
                if self.args.mask_flag:
                    ...
                    origin = self.interpolation_fn(origin, idx, num_workers=self.args.cpu)
                else: 
                    origin = sample["Input"]
...
#         for _, sample in enumerate(self.train_loader):
#             origin, idx = mask(sample["Input"], mask_mtd=self.config["mask_method"])
#             origin = self.interpolation_fn(origin, idx, num_workers=1)
#             origin = origin.float().to(self.device)
#             target = sample["Target"].float().to(self.device)
#             target_chunks = torch.chunk(target, self.config['train']['rollout_times'],dim=1)
...
#                 masked_origin, idx = mask(sample["Input"], mask_mtd=self.config["mask_method"])
...
#                     self.plot(epoch, origin_before_masked, masked_plot,
...
#     def plot(self, idx, origin, masked_origin, interpolated_origin, 
#              output_chunks, target_chunks):
        
#         rollout_times = self.config['train']['rollout_times']
#         seq_len = self.config['seq_length']

#         _, ax = plt.subplots(rollout_times * 2 + 3, seq_len + 1,
#                              figsize=(seq_len * 2 + 2, rollout_times * 4 + 6))
#         row_titles = ["Original input", "Masked input", "Interpolated input",
